...
class GemmaMultimodalProcessor:
    ...
    def __init__(self):
        ...
        self.model = Gemma3ForConditionalGeneration.from_pretrained(model_id, device_map="auto", torch_dtype=torch.bfloat16)
        self.processor = AutoProcessor.from_pretrained(model_id)
    ...
    async def generate_streaming(self, text):
        async with self.lock:
            try:
                ...
                streamer = TextIteratorStreamer(self.processor.tokenizer, skip_special_tokens=True, skip_prompt=True)generation_kwargs = dict(
                    **inputs,
