...
class AudioSegmentDetector:
    ...
    async def add_audio(self, audio_bytes):
        ...
        async with self.lock: # Protects audio_buffer and related state
                ...
                if not self.is_speech_active and energy > self.energy_threshold:
                    ...
                    if energy > self.energy_threshold:
                        ...
                        if self.silence_counter >= self.silence_samples:
                            ...
                            reset_speech_state = True
                            # Trim buffer AFTER potentially cancelling, just before returning
                            trim_index = speech_end_idx 
                            
                            if len(speech_segment) >= self.min_speech_samples * 2:
                                segment_to_process = speech_segment
                                logger.info(f"Speech segment detected (silence end): {len(speech_segment)/2/self.sample_rate:.2f}s")
                            else:
                                # Segment too short, just reset state and trim
                                logger.info(f"Short speech segment ignored ({len(speech_segment)/2/self.sample_rate:.2f}s)")
                                self.audio_buffer = self.audio_buffer[trim_index:] # Trim here if no processing
                            ...
                            speech_segment = bytes(self.audio_buffer[self.speech_start_idx : segment_end_idx])

                            # Don't reset silence counter, but update start index for next potential segment
                            self.speech_start_idx = segment_end_idx
                            # No reset_speech_state = True here, as speech continues
                            # No buffer trim here

                            segment_to_process = speech_segment
                            logger.info(f"Max duration speech segment detected: {len(speech_segment)/2/self.sample_rate:.2f}s")

                # --- Cancellation Logic ---
                if segment_to_process:
                    self.segments_detected += 1
                    
                    # Check if TTS is playing *outside* the main buffer lock if possible,
                    # but definitely before calling cancel_current_tasks while holding tts_lock.
                    needs_cancel = False
                    async with self.tts_lock:
                        if self.tts_playing:
                            needs_cancel = True
                    
                    # Perform cancellation *after* releasing tts_lock
                    if needs_cancel:
                        logger.info("Detected new speech segment while TTS might be active. Cancelling ongoing tasks.")
                        await self.cancel_current_tasks() # This internally sets tts_playing = False

                    # Now add segment to the queue
                    await self.segment_queue.put(segment_to_process)

                    # If we ended due to silence, trim the buffer now
                    if reset_speech_state:
                        self.audio_buffer = self.audio_buffer[trim_index:]

                    # Reset speech detection state if needed (only on silence end)
                    if reset_speech_state:
                        self.is_speech_active = False
                        self.silence_counter = 0

                    return segment_to_process # Indicate a segment was processed
